function [rpos_ecef, delta_t] = LS_single_diff(meas_temp, init_pos_ecef, DGPS_ONLY)
if nargin<3
    DGPS_ONLY = 0;
end
%% options
...
for i=1:meas_temp.num_sats
    ...
    if meas_temp.Sat_state(prn).sat_valid
        if meas_temp.Sat_state(prn).locked && (~DGPS_ONLY)
            Meas(i) = meas_temp.Sat_state(prn).ph_l1_rng;
            R_meas(i) = meas_temp.Sat_state(prn).R_ph;
                ...
                Meas(i) = meas_temp.Sat_state(prn).sd_code_l1;
                R_meas(i) = meas_temp.Sat_state(prn).R_cd;
                disp(['At time ', num2str(meas_temp.imu_tm),' Sv ', num2str(prn), ' use sd_code_l1']);
            else
                Meas(i) = meas_temp.Sat_state(prn).code_l1;
                R_meas(i) = meas_temp.Sat_state(prn).R_cd;
                disp(['At time ', num2str(meas_temp.imu_tm),' Sv ', num2str(prn), ' use code_l1']);
            end
        end
    else
