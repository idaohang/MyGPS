function [buffer] = read_GPS_old_meas_log(gps_meas, start_time, end_time)
NUM_SATS = 15;
NUM_PRNS = 32;
MAX_AGE_CD = 100;

if nargin < 2
	start_time = 0;
    end_time = 1000;
else if nargin < 3
        end_time = 1000;
    end
end

i = 1;
% push buffer from the starting time
while ( gps_meas(i,1)<start_time )
    i=i+1;
end
j=i+1;
while ( gps_meas(j,1)<end_time )
    j=j+1;
end
buff_log = gps_meas(i:j,:);
buff_size = length( buff_log(:,1) );

for index = 1:buff_size
    data = buff_log(index,:);
    count = 1;
    meas_temp.pps_tm = data(count);
    count=count+1;
    meas_temp.imu_tm = data(count);
    count=count+1;
%     meas_temp.tow = data(count);
%     count=count+1;
    meas_temp.num_sats = data(count);
    count=count+1;
%     meas_temp.num_locked = data(count);
%     count=count+1;
%     meas_temp.num_dgps = data(count);
%     count=count+1;

%     meas_temp.prnlist = zeros(meas_temp.num_sats,1);
%     for i=1:NUM_SATS
%         meas_temp.prnlist(i) = data(count);
%         count=count+1;
%     end
    for i = 1 : meas_temp.num_sats
        prn = data(count);
        meas_temp.Sat_state(prn).prn_num = data(count);
        count=count+1;
        meas_temp.Sat_state(prn).sat_valid = 1;
        meas_temp.Sat_state(prn).sv_pos_ecef(1) = data(count);
        count=count+1;
        meas_temp.Sat_state(prn).sv_pos_ecef(2) = data(count);
        count=count+1;
        meas_temp.Sat_state(prn).sv_pos_ecef(3) = data(count);
        count=count+1;
        meas_temp.Sat_state(prn).sv_vel_ecef(1) = data(count);
        count=count+1;
        meas_temp.Sat_state(prn).sv_vel_ecef(2) = data(count);
        count=count+1;
        meas_temp.Sat_state(prn).sv_vel_ecef(3) = data(count);
        count =count+1;
        meas_temp.Sat_state(prn).dgps_age = data(count);
        count=count+1;
        if meas_temp.Sat_state(prn).dgps_age>=0 && meas_temp.Sat_state(i).dgps_age<MAX_AGE_CD
            meas_temp.Sat_state(prn).sd_code_l1 = data(count);
            count=count+1;
            meas_temp.Sat_state(prn).sd_code_l2 = data(count);
            count=count+1;
            meas_temp.Sat_state(prn).sd_phase_l1 = data(count);
            count=count+1;
            meas_temp.Sat_state(prn).sd_phase_l2 = data(count);
            count=count+1;
        else
            meas_temp.Sat_state(prn).code_l1 = data(count);
            count=count+1;
            meas_temp.Sat_state(prn).code_l2 = data(count);
            count=count+1;
            meas_temp.Sat_state(prn).phase_l1 = data(count);
            count=count+1;
            meas_temp.Sat_state(prn).phase_l2 = data(count);
            count=count+1;
        end
        meas_temp.Sat_state(prn).ph_l1_rng = data(count);
        count=count+1;
        meas_temp.Sat_state(prn).ph_l2_rng = data(count);
        count=count+1;
        meas_temp.Sat_state(i).doppler_l1 = data(count);
        count=count+1;
        meas_temp.Sat_state(i).doppler_l2 = data(count);
        count=count+1;
    end
    for i=1:NUM_PRNS
        meas_temp.Sat_state(i).locked = data(count);
        count=count+1;
        meas_temp.Sat_state(i).prn_num = data(count);
        count=count+1;
        meas_temp.Sat_state(i).aode = data(count);
        count=count+1;
        meas_temp.Sat_state(i).N_l1 = data(count);
        count=count+1;
        meas_temp.Sat_state(i).N_l2 = data(count);
        count=count+1;
        meas_temp.Sat_state(i).N_nr = data(count);
        count=count+1;
        meas_temp.Sat_state(i).N_wd = data(count);
        count=count+1;

        meas_temp.Sat_state(i).ph_nr_rng = data(count);
        count=count+1;
        meas_temp.Sat_state(i).ph_wd_rng = data(count);
        count=count+1;
        meas_temp.Sat_state(i).rcode_l1 = data(count);
        count=count+1;
        meas_temp.Sat_state(i).rcode_l2 = data(count);
        count=count+1;
        meas_temp.Sat_state(i).rphase_l1 = data(count);
        count=count+1;
        meas_temp.Sat_state(i).rphase_l2 = data(count);
        count=count+1;

        meas_temp.Sat_state(i).std_code_l1 = data(count);
        count=count+1;
        meas_temp.Sat_state(i).std_code_l2 = data(count);
        count=count+1;
        meas_temp.Sat_state(i).std_phase_l1 = data(count);
        count=count+1;
        meas_temp.Sat_state(i).std_phase_l2 = data(count);
        count=count+1;
        meas_temp.Sat_state(i).d_tsv = data(count);
        count=count+1;
        meas_temp.Sat_state(i).std_phase_l1 = data(count);
        count=count+1;
        meas_temp.Sat_state(i).std_phase_l2 = data(count);
        count=count+1;
        
        meas_temp.Sat_state(i).corr_code_l1 = data(count);
        count=count+1;
        meas_temp.Sat_state(i).corr_code_l2 = data(count);
        count=count+1;
        meas_temp.Sat_state(i).corr_phase_l1 = data(count);
        count=count+1;
        meas_temp.Sat_state(i).corr_phase_l2 = data(count);
        count=count+1;

        meas_temp.Sat_state(i).R_cd = data(count);
        count=count+1;
        meas_temp.Sat_state(i).R_ph = data(count);
        count=count+1;
        meas_temp.Sat_state(i).R_dplr = data(count);
        count=count+1;
    end
    if index == 1
        buffer(buff_size,1)=meas_temp;
        buffer(1)=meas_temp;
    else
        buffer(index)=meas_temp;
    end
end


