function [H, R, residual] = msc_gps_ins_output(p, rpos_ecef, meas_buff, K)
k=1;
meas_temp = meas_buff(k);
% rpos_dgps = LS_double_diff(meas_temp, rpos_ecef(3*k-2:3*k), 1);
% rpos_tp = p.R_ned2ecef' *( rpos_dgps - p.ecef_p_b);
[num_sat, Meas, R_meas, prn_dd] = msc_gps_calc_meas(meas_temp); % get the latest l1 phase in meters
%Meas = Meas*p.wave_l1;
...
for j=1:K
    ...
    for i=1:num_sat
        ...
        residual((i-1)*K+j) = Meas(i) - PRangei ;%- Nhat(i)*p.wave_l1;
