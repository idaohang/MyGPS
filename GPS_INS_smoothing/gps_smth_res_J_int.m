...
function [r, J, g, gps_res] = gps_smth_res_J_int(nav, data, Xhat_k, Phase_meas)
global Dual_Freq;
dual_freq = Dual_Freq;
p = nav.param;
...
for k = 1:size(nav.buff.index,1) 
    ...
    %[rYN, JYN, y_N] = accumulateRN(rYN, JYN, HN, RN, dyN, y_N, k, Ndim, p.wave_l1); %k
    [rYN, JYN, y_N] = accumulateRN1(rYN, JYN, HN, RN, dyN, y_N, k, Ndim, p); %k
...
function [rY, JY, y] = accumulateRN1(rY, JY, H, R, dy, y, k, Ndim, p)
global Dual_Freq;
dual_freq = Dual_Freq;
N = size(H, 2);
m = size(dy, 1);
