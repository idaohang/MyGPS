...
rP = SigmaP*gpsins_compute_dx(nav.param, Xhat_k(1), Xhat_k(1)); % the last input changed from nav.xhat0
rQ = zeros(N*K, 1);
rY = zeros(Y, 1);
...
if Doppler
    ...
    [H, R, dy] = msc_gps_ins_output(nav.param, Pos_ecef, data.gps(ii_gps-K+1:ii_gps), K, prn_list);
    if ~isempty(dy)
       SigmaR = chol(inv(R));
       rD = -SigmaR*dy;
       [a,b] = size(H);
